...
const alterarTextos = () => {
    const section = document.getElementById("Etapa01")
    const input1  = section.children[5].value
    const input2  = section.children[6].value
    const input3  = section.children[7].value
    const p1 = section.children[1]
    const p2 = section.children[2]
    const p3 = section.children[3]
    if(input1) p1.textContent = input1 
    if(input2) p2.textContent = input2 
    if(input3) p3.textContent = input3 
...
function adicionarItem() {
    ...
    const digito = parseInt(last.textContent.split(" ")[1])  
    console.log(digito)
    const novoLi = document.createElement("li")
    novoLi.textContent = `Item ${digito + 1}`
...
function mudarCorFundo() {
    ...
    inputs.forEach((input, b ) => inputs.item(b).style.backgroundColor = cor)
}
...
function mostrarImagemMaior(img) {
    ...
    img.setAttribute("src", img.attributes["src"].textContent.replace("100", "300"))
...
function validarFormulario() {
    ...
    function emailValido(){
        ...
        if (Arroba || Arroba + 2 != email.length - 1 || email[Arroba + 2] != ".")
        

        return eh
    }

    if(!nome || !cpfValido() || !emailValido() || senha.length < 8){
        eh = false
        erro.textContent = "Valor invalido"
    }
